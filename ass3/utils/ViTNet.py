...
class ViT(nn.Module):
    ...
    def forward(self, x):
        res = torch.zeros(x.shape[0], self.num_classes)
        for i in range(x.shape[0]):
            inputs = self.feature_extractor(images=x[i].cpu(), return_tensors="pt")
            inputs.to(self.device)
            outputs = self.pre_model(**inputs)
            cls_token = outputs.last_hidden_state[:,0,:] 
            res[i] = self.last_layer(cls_token)
